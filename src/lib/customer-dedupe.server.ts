...
export async function findDuplicates(
  supabase: any,
  data: DedupeInput,
): Promise<{ duplicates: (CustomerRow & { reason: string })[] }> {
  ...
  const exact: CustomerRow[] = [];
  const candidates: CustomerRow[] = [];
  for (const r of (rows ?? []) as CustomerRow[]) {
...
Return JSON with "matches": array of { id, confidence (0..1), reason }. Only include items with confidence >= 0.75. Empty array if none.`,
    ...
    const duplicates = pool
      ...
      .map((r) => ({ ...r, reason: reasonById.get(r.id) ?? "Possible duplicate" }));
    return { duplicates };
  } catch (e) {
