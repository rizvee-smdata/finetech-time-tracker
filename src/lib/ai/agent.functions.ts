        ...
        actions: {
          ...
          items: {
            ...
            properties: {
              ...
              subject: { type: "string" },
              body: { type: "string" },
              description: { type: "string", description: "Timer description if type=start_timer" },
...
${data.dataSnapshot ? `# Workspace snapshot\n${data.dataSnapshot}` : ""}`;
    ...
    if (!args) return { reply: "Sorry, I couldn't form a response.", actions: [] as unknown[] };
    try {
      const parsed = JSON.parse(args) as { reply: string; actions?: unknown[] };
      return { reply: parsed.reply, actions: parsed.actions ?? [] };
    } catch {
      return { reply: "Sorry, my response was malformed.", actions: [] as unknown[] };
    }
  });
